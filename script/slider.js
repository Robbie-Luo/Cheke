(function(){
	...
	function Slider($elem, option) {
		...
		this.$data=this.options.tip;
		this.$tip = $(".tip", $elem);
		this.$tipContent = $(".tipContent", $elem);
	...
	Slider.prototype = {
		...
		_mousemoveHandler: function(e) {
			if (this._allowDrag) {
				...
				var commonSingleWidth = (this._fullbarWidth - this.options.firstWidth - this.options.lastWidth) / (this._scale.length - 3)/this._scale[1].value.length;				
				if (this._$activeButton[0] == this.$leftButton[0]) {
					if (newLeft > this.$rightButton.position().left-commonSingleWidth) {
						newLeft = this.$rightButton.position().left-commonSingleWidth;
					}
				} else {
					if (newLeft < this.$leftButton.position().left+commonSingleWidth) {
						newLeft = this.$leftButton.position().left+commonSingleWidth;
					}
				}
				var validBarLeft = this.$validbar.position().left + this.$fullbar.position().left;
				var validBarWidth = this.$validbar.width();
		...
		_setTipContent: function() {
			...
			var min = null;
			var max = null;
		 if (this._beyondMax == true && value.rightValue == this._getValueByTempValue(this._max) && this._beyondMin == true && value.leftValue == this._getValueByTempValue(this._min)) {
				content = "不限"
				...
				content = value.rightValue + this._unit + "以内"
